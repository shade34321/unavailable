...
/**
 * A class created to access Nurse information create appointments and other
 * such Nurse functions
 * 
 * @author Shade
 * 
 */
...
public class Nurse {
	 ...
	 * @param patient
	 */
	public Nurse(String user, AVL<String, Info> information,
			AVL<String, Info> informationUser) {
	...
	public boolean canDeletePatient(String name) {
		...
		if (informationName.find(name) == null) {
			myLogger.log(Level.CONFIG,
					"Patient does not exist!.. Or miss typed... " + name);
	...
	public void createOrder(String user, int date, int time, String prescrip,
		...
		if (informationName.find(user) != null) {
			informationName.find(user).getOrders()
					.create(user, date, time, prescrip, labW, followUp, other);
	 ...
	 * @param prescrip
	 */
	public void cancelDoctorOrder(int date, int time, String name,
			String prescrip) {

		if (informationName.find(name) != null) {
			informationName.find(name).getOrders()
					.cancel(date, time, name, prescrip);
